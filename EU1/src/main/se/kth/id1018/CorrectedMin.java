...
public class CorrectedMin {
	public static int correctedMin( int[] element ) throws IllegalArgumentException {
		if ( element.length == 0 ) {
		...
		int i = 0;
		int j = 0;
		while ( antaletPar >= 1 ) // Original: while ( sekvens.length > 1 )
		{
			...
			if ( antaletOparadeElement == 1 ) {
				delsekvens[ j ] = sekvens[ i ]; // Originally: delsekvens[ j ] = sekvens[ sekvens.length - 1 ];
			}
